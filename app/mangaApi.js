...
export async function fetchMangaList(page = 1, limit = 20) {
  ...
  const url = `${BASE_URL}/manga?limit=${limit}&offset=${offset}&order[followedCount]=desc&availableTranslatedLanguage[]=en&hasAvailableChapters=true&contentRating[]=safe`;
...
export async function fetchMangaChapters(mangaId, page = 1, limit = 20) {
  const offset = (page - 1) * limit;
  const url = `${BASE_URL}/manga/${mangaId}/feed?limit=${limit}&offset=${offset}&translatedLanguage[]=en&order[chapter]=asc`;
  const res = await fetch(`${PROXY}${encodeURIComponent(url)}`);
...
export async function fetchMangaCovers(mangaList) {
  ...
  const url = `${BASE_URL}/cover?ids[]=${coverIds.join("&ids[]=")}`;
  const res = await fetch(`${PROXY}${encodeURIComponent(url)}`);
